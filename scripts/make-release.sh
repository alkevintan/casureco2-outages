#!/usr/bin/env bash
# Cut a CASURECO 2 Outages release. Bumps versionName/versionCode, builds, tags, pushes,
# and creates a GitHub Release with the APK attached.
#
# Usage:
#   scripts/make-release.sh 1.0.1
#   scripts/make-release.sh 1.0.1 --notes "Fixed parsing bug"
#   scripts/make-release.sh 1.0.1 --notes-file CHANGELOG.md
#   scripts/make-release.sh 1.0.1 --prerelease
#
# If neither --notes nor --notes-file is given, notes are auto-generated from
# `git log` since the previous tag.

set -euo pipefail

REPO="alkevintan/casureco2-outages"
...
if [[ -n "$(git status --porcelain)" ]]; then
    die "working tree dirty (modified or untracked files present) — commit or stash first"
fi
...
CURRENT_VERSION_CODE="$(grep -E '^\s*versionCode ' "$GRADLE_FILE" | awk '{print $3}')"
...
# macOS sed (BSD) needs '' after -i; GNU sed doesn't. Detect.
...
APK_DST="/tmp/CASURECO2Outages-$VERSION.apk"
cp "$APK_SRC" "$APK_DST"
